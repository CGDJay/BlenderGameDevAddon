...
selection_uv_mode = 'ISLAND'
selection_uv_loops = set()
selection_uv_pivot = 'MEDIAN'
...
def selection_store(bm=None, uv_layers=None, return_selected_UV_faces=False, return_selected_faces_edges=False, return_selected_faces_loops=False):
	...
	use_uv_sync = bpy.context.scene.tool_settings.use_uv_select_sync
	selection_uv_mode = bpy.context.scene.tool_settings.uv_select_mode

	contextViewUV = GetContextViewUV()
	if contextViewUV:
		selection_uv_pivot = contextViewUV['area'].spaces[0].pivot_point
		selection_uv_pivot_pos = contextViewUV['area'].spaces[0].cursor_location.copy()

	# Clear previous selection
	selection_vert_indexies.clear()
	selection_edge_indexies.clear()
	selection_face_indexies.clear()
	seam_edges.clear()
	...
	if selection_mode[0]:
		for vert in bm.verts:
			if vert.select:
				selection_vert_indexies.add(vert.index)
	if selection_mode[1]:
		for edge in bm.edges:
			if edge.select:
				selection_edge_indexies.add(edge.index)

	# Face selections (Loops)
	selection_uv_loops.clear()
	...
	for face in bm.faces:
		if face.select:
			selection_face_indexies.add(face.index)
		n_selected_loops = 0
		...
		for loop in face.loops:
			...
			if loop[uv_layers].select:
				n_selected_loops += 1
				selection_uv_loops.add( (face.index, loop.vert.index) )
				if return_selected_faces_edges or return_selected_faces_loops:
...
def selection_restore(bm = None, uv_layers = None, restore_seams=False):
	...
	bpy.context.scene.tool_settings.use_uv_select_sync = use_uv_sync
	bpy.context.scene.tool_settings.uv_select_mode = selection_uv_mode

	contextViewUV = GetContextViewUV()
	if contextViewUV:
		contextViewUV['area'].spaces[0].pivot_point = selection_uv_pivot
		bpy.ops.uv.cursor_set(contextViewUV, location=selection_uv_pivot_pos)
	...
	if selection_mode[0]:
		...
		for index in selection_vert_indexies:
			if index < len(bm.verts):
				bm.verts[index].select = True
	if selection_mode[1]:
		bm.edges.ensure_lookup_table()
		for index in selection_edge_indexies:
			if index < len(bm.edges):
				bm.edges[index].select = True
	bm.faces.ensure_lookup_table()
	for index in selection_face_indexies:
	...
	for uv_set in selection_uv_loops:
