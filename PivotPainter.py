...
class UE4_PivotPainterProperties(PropertyGroup):															# create property group for user options
    ...
    savetextures : BoolProperty( name = "Save Textures to folder", default = True, description = ("Will always OVERWRITE texture files with the same name\n\nSave textures to the specified folder location"))
...
class PPB_PT_panel:																			# THe panel in the UI
    ...
    def poll(cls, context):
        ...
        if bpy.context.preferences.addons[AddonName].preferences.bool_Enable_PivotPainter == True:
            return True
        else:
            return False
...
class PivotToVcol (bpy.types.Operator):
    
    bl_idname = "pivot.vcol"
    bl_label = "PivotToVcol"
    ...
    def execute (self, context):
        ...
        # for obj in objs list it will ger the bounding box of the the object and multiply is by object scale incase the user has not correctly applied scale before using the tool
        for obj in objs:
            scale = obj.scale
        ...
        # will next find the furthest distances for the center of the object by comparing the scale of the bounding box on each dimention 
        ...
        # (maybe writing a solution to compare it from active selection instead of world origin poisition)    
        ...
        for obj in bpy.context.selected_objects:
            ...
            #goes through all the faces of the Bmesh and assigns the new color
            for face in bm.faces:
                for loop in face.loops:
                    loop[color_layer] = color

            #Updates the Bmesh with the new changes        
            bm.to_mesh(obj.data)
