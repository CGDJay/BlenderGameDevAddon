...
class VERTEXCOLORMASTER_PT_MainPanel(Panel):
    ...
    def poll(cls, context):
        ...
        if bpy.context.preferences.addons[AddonName].preferences.bool_Enable_VPM == True:
            return True
        else:
            return False
...
def draw_src_dst_operations(context, layout, obj, settings):
    ...
    row.prop(settings, 'src_channel_id', text="")
    row.enabled = src_type == type_vcol and (dst_type == type_vcol or dst_type == type_vgroup)

    row.separator()

    row.prop(settings, 'dst_channel_id', text="")
    row.enabled = dst_type == type_vcol and (src_type == type_vcol or src_type == type_vgroup)
...
def draw_misc_operations(context, layout, obj, settings, mode='STANDARD', pie=False):
    ...
    # row = col.row(align=True)
    # row.operator('paint.edgeware', text="EdgeWare Vertex Colors")
