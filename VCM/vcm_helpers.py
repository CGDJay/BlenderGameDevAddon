...
def remap(value, min0, max0, min1, max1):
    # define redult variable as max minus the min
    r0 = max0 - min0
    #if the result is equal to 0 then set the result as min
    if r0 == 0:
        return min1
    r1 = max1 - min1
...
def channel_id_to_idx(id):
    # define channel ids as the correct index number
    if id is red_id:
        return 0
    if id is green_id:
        return 1
...
# if vcol layer name starts with then correct prefix and contains more letters than the prefix plus 3 additional character
