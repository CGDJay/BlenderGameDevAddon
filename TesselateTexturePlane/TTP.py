...
class TESS_props_group(PropertyGroup):
    ...
    default=25, min=0.0, max=100.0, step=1, precision=1, subtype='PERCENTAGE', unit='NONE')

    pix_margin : IntProperty(name="Pixel margin", 
    description="0=disabled, Dilate contour around shape", 
    default=15, min=0, max=800, soft_min=0, soft_max=50, step=1, subtype='PIXEL')
    ...
    default=0, min=0, max=99, step=1)#100 run forever
...
def tesselate(obj, contour_only=False, simplify=0.00150, pix_margin=15, min_angles=0, aeration=0.005, external_only=False, pix_clean=0, true_delaunay=False, algo_inc=False, uv_mask=True):
...
class TESS_OT_tesselate_plane(Operator):
    ...
    min_angles : IntProperty(name="Density", 
    description="0=disabled, Increase tessselation density", 
    default=0, min=0, max=99, step=1)#100 run forever
